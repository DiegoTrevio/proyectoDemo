"""Memory Manager — orchestrates the 4-layer memory system.

Integration flow:
  BEFORE execution:
    1. OpenViking L0 for quick context (~100 tokens)
    2. Mem0 for user preferences and history
    3. Graphiti for timestamped facts
    4. If more detail needed: escalate to L1/L2
  AFTER execution:
    Save new learnings to all 4 layers
"""
...
async def gather_context(
    ...
    # ── Layer 1: OpenViking L0 (quick context) ────────────────────────
    try:
    ...
    # ── Layer 2: Mem0 (user preferences + history) ────────────────────
    try:
        memories = mem0.search_memory(user_id, goal, limit=3)
        if memories:
            context["mem0_memories"] = [{"content": m.content, "score": m.score} for m in memories]
    ...
    # ── Layer 3: Graphiti (timestamped facts) ─────────────────────────
    try:
...
async def save_learnings(
    ...
    """
    status = {"openviking": False, "mem0": False, "graphiti": False, "redis": False}
    now = datetime.now(timezone.utc)

    # ── Layer 1: OpenViking — store as resource ───────────────────────
    try:
        summary = f"Task: {goal}\nResult: {result[:2000]}"
    ...
    # ── Layer 2: Mem0 — store as user memory ──────────────────────────
    try:
        mem0.add_memory(
            user_id=user_id,
        ...
        # Also store agent-level learning
