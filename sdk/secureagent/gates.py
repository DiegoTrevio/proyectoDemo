...
# Type for approval callback: receives ApprovalRequest, returns bool
...
class ApprovalGate:
    ...
    async def check(
        ...
        request = ApprovalRequest(
            id=f"apr_{self._counter}_{uuid.uuid4().hex[:6]}",
        ...
        if not self._callback:
            logger.error("No approval callback configured — rejecting HIGH risk action")
        ...
        try:
            ...
            self._history.append(request)
            logger.warning("Approval timeout for %s", tool_name)
            return False
